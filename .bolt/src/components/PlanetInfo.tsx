...
import { Globe, Users, Database, Zap, Shield, Flag, Crosshair, Anchor, Building2, Rocket } from 'lucide-react';
...
interface PlanetProps {
  ...
  onUserUpdate: () => void;
  onPlanetUpdate: (updatedPlanet: Planet) => void;
}
  ...
  const confirmTransaction = async () => {
    ...
    try {
      await runTransaction(db, async (transaction) => {
        ...
        const planetDoc = await transaction.get(planetRef);

        if (!userDoc.exists() || !planetDoc.exists()) {
        ...
        if (planetData.resources[selectedResource as keyof typeof planetData.resources] < amount) {
          throw "Not enough resources on the planet!";
        }

        // Update user data
        transaction.update(userRef, {
          credits: userData.credits - cost,
          [`resources.${selectedResource}`]: userData.resources[selectedResource] + amount
        });

        // Update planet data
        transaction.update(planetRef, {
          [`resources.${selectedResource}`]: planetData.resources[selectedResource as keyof typeof planetData.resources] - amount
        });

        // Update local planet state
        const updatedPlanet = {
          ...planet,
          resources: {
            ...planet.resources,
            [selectedResource]: planet.resources[selectedResource as keyof typeof planet.resources] - amount
          }
        };
        onPlanetUpdate(updatedPlanet);
      });
